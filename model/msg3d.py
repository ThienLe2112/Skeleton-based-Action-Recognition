...
class Model(nn.Module):
    ...
    def forward(self, x):
        ...
        ##Edit Code Begin 4
        x = F.relu(self.sgcn1([x,xx]) + self.gcn3d1([x,xx]), inplace=True)
        x = self.tcn1(x)

        x = F.relu(self.sgcn2([x,xx]) + self.gcn3d2([x,xx]), inplace=True)
        x = self.tcn2(x)

        x = F.relu(self.sgcn3([x,xx]) + self.gcn3d3([x,xx]), inplace=True)
        x = self.tcn3(x)
